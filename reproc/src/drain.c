...
{
  return reproc_drain_bufsize(process, out, err, 4096);
}

int reproc_drain_bufsize(reproc_t *process, reproc_sink out, reproc_sink err, const size_t bufSize)
{
  ...
  for (;;) {
    ...
    r = reproc_read(process, stream, buffer, bufSize);
