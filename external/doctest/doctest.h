...
enum Enum {
  ...
  DT_FAST_WARN_UNARY_FALSE = is_normal | is_fast | is_false | is_unary |
                             is_warn,
  DT_FAST_CHECK_UNARY_FALSE = is_normal | is_fast | is_false | is_unary |
                              is_check,
  DT_FAST_REQUIRE_UNARY_FALSE = is_normal | is_fast | is_false | is_unary |
                                is_require
};
} // namespace assertType
...
#define DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(id, anon, ...)             \
  DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_CAT(anon, DUMMY)) = []() {                \
    DOCTEST_CAT(id, ITERATOR)<__VA_ARGS__> DOCTEST_UNUSED                      \
...
#define DOCTEST_SUBCASE(name)                                                  \
  if (const doctest::detail::Subcase &                                         \
          DOCTEST_ANONYMOUS(_DOCTEST_ANON_SUBCASE_)                            \
              DOCTEST_UNUSED = doctest::detail::Subcase(name, __FILE__,        \
                                                        __LINE__))
...
struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats {
  ...
  std::vector<std::vector<String>> filters = decltype(filters)(
      9); // 9 different filters
